...
# change type
as.factor(dat$ID) # doesn't change data set object
dat$ID = as.factor(dat$ID) # rewrites original variable as new variable as factor
dat$ID2 = as.factor(dat$ID) # creates new variable as factor
dat$meim01 = as.numeric(dat$meim)
glimpse(dat)
...
sum(is.na(dat[dat$ID == "MCAE16002",])) # identifies number of missing values for subject

sum(is.na(dat)) # total missing data for whole data set
sum(is.na(dat$meim01)) # total missing data for one variable
...
# max, min, mean
max(dat$meim_mean, na.rm = T) # need to include na.rm = T as argument
min(dat$meim_mean, na.rm = T)
mean(dat$meim_mean, na.rm = T)
